 ...
 * clusters, attributes, commands) using code. It manages a list of EndpointProviderInterface
 * objects, each representing an endpoint on the device.
 *
 * Expected Usage:
 * 1. Instantiate an EndpointProviderInterface implementation.
 ...
 *
 * Lifecycle:
 * - The CodeDrivenDataModelProvider stores raw pointers to EndpointProviderInterface.
...
public:
     ...
     * Any EndpointProviderInterface instance MUST outlive the CodeDrivenDataModelProvider it is registered with.
     * Similarly, EndpointProviderRegistration objects passed to AddEndpoint must outlive their registration.
     */
    CHIP_ERROR AddEndpoint(EndpointProviderRegistration & registration);
...
private:
    ...
    EndpointProviderRegistry mEndpointProviderRegistry;
