...
CHIP_ERROR StartupServerClusters(EndpointProviderInterface * endpointProvider, ServerClusterContext & serverClusterContext)
{
...
CHIP_ERROR ShutdownServerClusters(EndpointProviderInterface * endpointProvider)
{
    VerifyOrReturnError(endpointProvider != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
...
CHIP_ERROR CodeDrivenDataModelProvider::Startup(DataModel::InteractionModelContext context)
{
    ...
    bool had_failure = false;
    for (auto * endpointProvider : mEndpointProviderRegistry)
    {
                                                                         ...
                                                                         AttributeValueEncoder & encoder)
{
    ...
    VerifyOrReturnError(epProvider != nullptr, Status::UnsupportedEndpoint);
...
{
    size_t count = 0;
    for (const auto * endpointProvider : mEndpointProviderRegistry)
    {
...
CHIP_ERROR CodeDrivenDataModelProvider::AddEndpoint(EndpointProviderRegistration & registration)
{
    VerifyOrReturnError(registration.endpointProviderInterface != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
    ReturnErrorOnFailure(mEndpointProviderRegistry.Register(registration));
    ...
    return StartupServerClusters(registration.endpointProviderInterface, *mServerClusterContext);
}

CHIP_ERROR CodeDrivenDataModelProvider::RemoveEndpoint(EndpointId endpointId)
{
    EndpointProviderInterface * provider = mEndpointProviderRegistry.Get(endpointId);
    ...
    return mEndpointProviderRegistry.Unregister(endpointId);
}

EndpointProviderInterface * CodeDrivenDataModelProvider::GetEndpointProvider(EndpointId endpointId)
{
    return mEndpointProviderRegistry.Get(endpointId);
}
...
void CodeDrivenDataModelProvider::SetPersistentStorageDelegate(PersistentStorageDelegate * delegate)
{
    ...
    {
        // If the provider has been started, we should re-startup the server clusters.
        for (auto * endpointProvider : mEndpointProviderRegistry)
        {
            CHIP_ERROR err = StartupServerClusters(endpointProvider, *mServerClusterContext);
            if (err != CHIP_NO_ERROR)
            {
                ChipLogError(DataManagement, "Failed to re-startup server clusters for endpoint %u: %" CHIP_ERROR_FORMAT,
...
PersistentStorageDelegate * CodeDrivenDataModelProvider::GetPersistentStorageDelegate()
{
