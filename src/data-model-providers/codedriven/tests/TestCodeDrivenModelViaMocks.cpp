...
using chip::app::SpanEndpointProvider;
...
namespace {
// Define constants and macros used by gTestNodeConfig
...
// Static instances that will be used to initialize the CodeDrivenDataModelProvider based on the Ember Mock above
ServerClusterShim ep0ServerCluster0({ { gTestNodeConfig.endpoints[0].id, gTestNodeConfig.endpoints[0].clusters[0].id } });
ServerClusterShim ep0ServerCluster1({ { gTestNodeConfig.endpoints[0].id, gTestNodeConfig.endpoints[0].clusters[1].id } });

ServerClusterShim ep1ServerCluster0({ { gTestNodeConfig.endpoints[1].id, gTestNodeConfig.endpoints[1].clusters[0].id } });
ServerClusterShim ep1ServerCluster1({ { gTestNodeConfig.endpoints[1].id, gTestNodeConfig.endpoints[1].clusters[1].id } });
ServerClusterShim ep1ServerCluster2({ { gTestNodeConfig.endpoints[1].id, gTestNodeConfig.endpoints[1].clusters[2].id } });

ServerClusterShim ep2ServerCluster0({ { gTestNodeConfig.endpoints[2].id, gTestNodeConfig.endpoints[2].clusters[0].id } });
ServerClusterShim ep2ServerCluster1({ { gTestNodeConfig.endpoints[2].id, gTestNodeConfig.endpoints[2].clusters[1].id } });
ServerClusterShim ep2ServerCluster2({ { gTestNodeConfig.endpoints[2].id, gTestNodeConfig.endpoints[2].clusters[2].id } });
ServerClusterShim ep2ServerCluster3({ { gTestNodeConfig.endpoints[2].id, gTestNodeConfig.endpoints[2].clusters[3].id } });

// For kMockEndpoint1 (gTestNodeConfig.endpoints[0]):
// Server Clusters: MockClusterId(1), MockClusterId(2)
// Client Clusters: MockClusterId(3), MockClusterId(4)
std::array<ServerClusterInterface *, 2> ep0ServerClusterInterfaces = {
    &ep0ServerCluster0,
    &ep0ServerCluster1,
};
const std::array<ClusterId, 2> ep0ClientClusters = { { MockClusterId(3), MockClusterId(4) } };

// For kMockEndpoint2 (gTestNodeConfig.endpoints[1]):
// Server Clusters: MockClusterId(1), MockClusterId(2), MockClusterId(3)
// Client Clusters: MockClusterId(3), MockClusterId(4)
std::array<ServerClusterInterface *, 3> ep1ServerClusterInterfaces = {
    &ep1ServerCluster0,
    &ep1ServerCluster1,
    &ep1ServerCluster2,
};
const std::array<ClusterId, 2> ep1ClientClusters = { { MockClusterId(3), MockClusterId(4) } };

// For kMockEndpoint3 (gTestNodeConfig.endpoints[2]):
// Server Clusters: MockClusterId(1), MockClusterId(2), MockClusterId(3), MockClusterId(4)
// Client Clusters: None
std::array<ServerClusterInterface *, 4> ep2ServerClusterInterfaces = {
    &ep2ServerCluster0,
    &ep2ServerCluster1,
    &ep2ServerCluster2,
    &ep2ServerCluster3,
};
const std::array<ClusterId, 0> ep2ClientClusters = {};

const std::array<DataModel::DeviceTypeEntry, 3> ep0DeviceTypes = { {
    { kDeviceTypeId1, kDeviceTypeId1Version },
    { kDeviceTypeId2, kDeviceTypeId2Version },
    { kDeviceTypeId3, kDeviceTypeId3Version },
} };
const std::array<DataModel::DeviceTypeEntry, 1> ep1DeviceTypes = { {
    { kDeviceTypeId2, kDeviceTypeId2Version },
} };
const std::array<DataModel::DeviceTypeEntry, 0> ep2DeviceTypes = {};

const std::array<SemanticTag, 3> ep0SemanticTags = { {
    { MakeNullable(VendorId::TestVendor1), kNamespaceID1, kTag1, MakeOptional(MakeNullable(CharSpan::fromCharString(kLabel1))) },
    { Nullable<VendorId>(), kNamespaceID2, kTag2, MakeOptional(MakeNullable(CharSpan::fromCharString(kLabel2))) },
    { MakeNullable(VendorId::TestVendor3), kNamespaceID3, kTag3, NullOptional },
} };
const std::array<SemanticTag, 0> ep1SemanticTags = { {} };
const std::array<SemanticTag, 0> ep2SemanticTags = { {} };

SpanEndpointProvider endpointProvider0 = []() {
    auto build_variant =
        SpanEndpointProvider::Builder(gTestNodeConfig.endpoints[0].id)
            .SetComposition(static_cast<DataModel::EndpointCompositionPattern>(gTestNodeConfig.endpoints[0].composition))
            .SetServerClusters(Span(ep0ServerClusterInterfaces.data(), ep0ServerClusterInterfaces.size()))
            .SetClientClusters(Span(ep0ClientClusters.data(), ep0ClientClusters.size()))
            .SetSemanticTags(Span(ep0SemanticTags.data(), ep0SemanticTags.size()))
            .SetDeviceTypes(Span(ep0DeviceTypes.data(), ep0DeviceTypes.size()))
            .build();
    VerifyOrDie(std::holds_alternative<SpanEndpointProvider>(build_variant)); // Assuming build is successful for test setup
    return std::get<SpanEndpointProvider>(std::move(build_variant));
}();

SpanEndpointProvider endpointProvider1 = []() {
    auto build_variant =
        SpanEndpointProvider::Builder(gTestNodeConfig.endpoints[1].id)
            .SetComposition(static_cast<DataModel::EndpointCompositionPattern>(gTestNodeConfig.endpoints[1].composition))
            .SetServerClusters(Span(ep1ServerClusterInterfaces.data(), ep1ServerClusterInterfaces.size()))
            .SetClientClusters(Span(ep1ClientClusters.data(), ep1ClientClusters.size()))
            .SetSemanticTags(Span(ep1SemanticTags.data(), ep1SemanticTags.size()))
            .SetDeviceTypes(Span(ep1DeviceTypes.data(), ep1DeviceTypes.size()))
            .build();
    VerifyOrDie(std::holds_alternative<SpanEndpointProvider>(build_variant)); // Assuming build is successful for test setup
    return std::get<SpanEndpointProvider>(std::move(build_variant));
}();

SpanEndpointProvider endpointProvider2 = []() {
    auto build_variant =
        SpanEndpointProvider::Builder(gTestNodeConfig.endpoints[2].id)
            .SetComposition(static_cast<DataModel::EndpointCompositionPattern>(gTestNodeConfig.endpoints[2].composition))
            .SetServerClusters(Span(ep2ServerClusterInterfaces.data(), ep2ServerClusterInterfaces.size()))
            .SetClientClusters(Span(ep2ClientClusters.data(), ep2ClientClusters.size()))
            .SetSemanticTags(Span(ep2SemanticTags.data(), ep2SemanticTags.size()))
            .SetDeviceTypes(Span(ep2DeviceTypes.data(), ep2DeviceTypes.size()))
            .build();
    VerifyOrDie(std::holds_alternative<SpanEndpointProvider>(build_variant)); // Assuming build is successful for test setup
    return std::get<SpanEndpointProvider>(std::move(build_variant));
}();

// This is the final array of EndpointProvider pointers that will be used in tests
// TODO: Make it easier to define the data model without having to spell out everything in detail
std::array<EndpointProviderInterface *, 3> epProvidersMatchingTestNodeConfig = {
    &endpointProvider0,
    &endpointProvider1,
    &endpointProvider2,
};
...
struct TestCodeDrivenModelViaMocks : public ::testing::Test
{
...
class CodeDrivenDataModelProviderWithStorage : public CodeDrivenDataModelProvider
{
public:
    CodeDrivenDataModelProviderWithStorage(
        const MockNodeConfig & nodeConfig                   = gTestNodeConfig,
        Span<EndpointProviderInterface *> endpointProviders = Span(epProvidersMatchingTestNodeConfig.data(),
                                                                   epProvidersMatchingTestNodeConfig.size())) :
        mUseMockNodeConfig(nodeConfig), // RAII for Set/ResetMockNodeConfig
        mContext({
    ...
    {
        // Initialize the provider with data from nodeConfig
        for (EndpointProviderInterface * ep : endpointProviders)
        {
            // Create and store EndpointProviderRegistration objects
            mOwnedRegistrations.emplace_back(*ep);
        }
        for (auto & reg : mOwnedRegistrations)
        {
            VerifyOrDie(AddEndpoint(reg) == CHIP_NO_ERROR);
        }
        VerifyOrDie(Startup(mContext) == CHIP_NO_ERROR); // Startup after all endpoints are added
    }

    ~CodeDrivenDataModelProviderWithStorage() { Shutdown(); }

    InteractionModelContext & Context() { return mContext; }
...
private:
    UseMockNodeConfig mUseMockNodeConfig; // Handles SetMockNodeConfig and ResetMockNodeConfig
public:                                   // Made public for test access
    TestEventGenerator mEventGenerator;
    TestProviderChangeListener mChangeListener;
    TestActionContext mActionContext;
    std::list<EndpointProviderRegistration> mOwnedRegistrations;
...
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, InitializeCodeDrivenDataModelFromMockConfig)
{
    CodeDrivenDataModelProviderWithStorage provider;

    // Verify Endpoints
    ReadOnlyBufferBuilder<EndpointEntry> endpointBuilder;
    ASSERT_EQ(provider.Endpoints(endpointBuilder), CHIP_NO_ERROR);
    auto retrieved_endpoints_span = endpointBuilder.TakeBuffer();
    ASSERT_EQ(retrieved_endpoints_span.size(), gTestNodeConfig.endpoints.size());

    // Convert Span to std::vector to sort, as the order from the provider might not match gTestNodeConfig.
    std::vector<EndpointEntry> sorted_retrieved_endpoints(retrieved_endpoints_span.begin(), retrieved_endpoints_span.end());
    std::sort(sorted_retrieved_endpoints.begin(), sorted_retrieved_endpoints.end(),
              [](const EndpointEntry & a, const EndpointEntry & b) { return a.id < b.id; });

    for (const auto & retrieved_ep : sorted_retrieved_endpoints)
    {
        const MockEndpointConfig * mock_config = gTestNodeConfig.endpointById(retrieved_ep.id);
        ASSERT_NE(mock_config, nullptr) << "Endpoint ID " << retrieved_ep.id << " not found in gTestNodeConfig";

        EXPECT_EQ(retrieved_ep.id, mock_config->id);
        EXPECT_EQ(retrieved_ep.compositionPattern, static_cast<EndpointCompositionPattern>(mock_config->composition));
        // In this test setup, top-level endpoints provided by SpanEndpointProvider default to kInvalidEndpointId for parentId.
        EXPECT_EQ(retrieved_ep.parentId, kInvalidEndpointId);
    }

    // Verify one endpoint in detail (e.g., kMockEndpoint1)
    const MockEndpointConfig * mockEp1 = gTestNodeConfig.endpointById(kMockEndpoint1);
    ASSERT_NE(mockEp1, nullptr);

    // Verify Device Types for kMockEndpoint1
    ReadOnlyBufferBuilder<DeviceTypeEntry> deviceTypeBuilder;
    ASSERT_EQ(provider.DeviceTypes(kMockEndpoint1, deviceTypeBuilder), CHIP_NO_ERROR);
    auto deviceTypes = deviceTypeBuilder.TakeBuffer();
    ASSERT_EQ(deviceTypes.size(), mockEp1->deviceTypes().size());
    for (size_t i = 0; i < deviceTypes.size(); ++i)
    {
        EXPECT_EQ(deviceTypes[i], mockEp1->deviceTypes()[i]);
    }

    // Verify Semantic Tags for kMockEndpoint1
    ReadOnlyBufferBuilder<SemanticTag> tagBuilder;
    ASSERT_EQ(provider.SemanticTags(kMockEndpoint1, tagBuilder), CHIP_NO_ERROR);
    auto tags = tagBuilder.TakeBuffer();
    ASSERT_EQ(tags.size(), mockEp1->semanticTags().size());
    for (size_t i = 0; i < tags.size(); ++i)
    {
        // SemanticTag comparison needs custom logic if operator== is not defined
        EXPECT_EQ(tags[i].mfgCode, mockEp1->semanticTags()[i].mfgCode);
        EXPECT_EQ(tags[i].namespaceID, mockEp1->semanticTags()[i].namespaceID);
        EXPECT_EQ(tags[i].tag, mockEp1->semanticTags()[i].tag);
        EXPECT_EQ(tags[i].label.HasValue(), mockEp1->semanticTags()[i].label.HasValue());
        if (tags[i].label.HasValue())
        {
            EXPECT_EQ(tags[i].label.Value().IsNull(), mockEp1->semanticTags()[i].label.Value().IsNull());
            if (!tags[i].label.Value().IsNull())
            {
                EXPECT_TRUE(tags[i].label.Value().Value().data_equal(mockEp1->semanticTags()[i].label.Value().Value()));
            }
        }
    }

    // Verify Server Clusters for kMockEndpoint1
    ReadOnlyBufferBuilder<ServerClusterEntry> serverClusterBuilder;
    ASSERT_EQ(provider.ServerClusters(kMockEndpoint1, serverClusterBuilder), CHIP_NO_ERROR);
    auto serverClusters        = serverClusterBuilder.TakeBuffer();
    size_t expectedServerCount = 0;
    for (const auto & clusterConfig : mockEp1->clusters)
    {
        if (clusterConfig.emberCluster()->mask & MATTER_CLUSTER_FLAG_SERVER)
        {
            bool found = false;
            for (const auto & entry : serverClusters)
            {
                if (entry.clusterId == clusterConfig.id)
                {
                    found = true;
                    // Further checks like data version could be added if FakeDefaultServerCluster provided them
                    break;
                }
            }
            EXPECT_TRUE(found);
    ...
    EXPECT_EQ(serverClusters.size(), expectedServerCount);

    // Verify Client Clusters for kMockEndpoint1
    ReadOnlyBufferBuilder<ClusterId> clientClusterBuilder;
    ASSERT_EQ(provider.ClientClusters(kMockEndpoint1, clientClusterBuilder), CHIP_NO_ERROR);
    auto clientClusters        = clientClusterBuilder.TakeBuffer();
    size_t expectedClientCount = 0;
    for (const auto & clusterConfig : mockEp1->clusters)
    {
        if (clusterConfig.emberCluster()->mask & MATTER_CLUSTER_FLAG_CLIENT)
        {
            bool found = false;
            for (const auto & clientId : clientClusters)
            {
                if (clientId == clusterConfig.id)
                {
                    found = true;
                    break;
                }
    ...
    EXPECT_EQ(clientClusters.size(), expectedClientCount);
}

TEST_F(TestCodeDrivenModelViaMocks, IterateOverServerClusters)
{
    CodeDrivenDataModelProviderWithStorage model;
    ...
    EXPECT_EQ(model.ServerClusters(kEndpointIdThatIsMissing, builder), CHIP_ERROR_NOT_FOUND);
...
TEST_F(TestCodeDrivenModelViaMocks, IterateOverClientClusters)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, IterateOverAttributes)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, FindAttribute)
{
    CodeDrivenDataModelProviderWithStorage model;
    ...
    info.emplace(std::move(*finder.Find(ConcreteAttributePath(kMockEndpoint2, MockClusterId(2), MockAttributeId(2)))));
    ASSERT_TRUE(info.has_value());
    ...
    info.emplace(std::move(*finder.Find(ConcreteAttributePath(kMockEndpoint3, MockClusterId(3), kReadOnlyAttributeId))));
    ASSERT_TRUE(info.has_value());
    EXPECT_FALSE(info->HasFlags(AttributeQualityFlags::kListAttribute));       // NOLINT(bugprone-unchecked-optional-access)
    EXPECT_EQ(info->GetReadPrivilege(), chip::Access::Privilege::kAdminister); // NOLINT(bugprone-unchecked-optional-access)
...
TEST_F(TestCodeDrivenModelViaMocks, GlobalAttributeInfo)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, IterateOverGeneratedCommands)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, AcceptedGeneratedCommandsOnInvalidEndpoints)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, CommandHandlerInterfaceCommandHandling)
{
...
TEST_F(TestCodeDrivenModelViaMocks, ReadForInvalidGlobalAttributePath)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeInvalidRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AccessInterfaceUnsupportedRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadInt8U)
{
    TestEmberScalarTypeRead<uint8_t, ZCL_INT8U_ATTRIBUTE_TYPE>(0x12);
}

TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadNulls)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadErrorReading)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadNullOctetString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadOctetString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadLongOctetString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadShortString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeReadLongString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AttributeAccessInterfaceStructRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AttributeAccessInterfaceReadError)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AttributeAccessInterfaceListRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AttributeAccessInterfaceListOverflowRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, AttributeAccessInterfaceListIncrementalRead)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteBasicTypes)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteInvalidValueToNullable)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberTestWriteReservedNullPlaceholderToNullable)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberTestWriteOutOfRepresentableRangeOddIntegerNonNullable)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberTestWriteOutOfRepresentableRangeOddIntegerNullable)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteBasicTypesLowestValue)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteNulls)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteShortString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteLongStringOutOfBounds)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteLongString)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteNullableLongStringValue)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteLongNullableStringNull)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteShortBytes)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteLongBytes)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberAttributeWriteDataVersion)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, WriteToInvalidPath)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, WriteToGlobalAttribute)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberWriteFailure)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberWriteAttributeAccessInterfaceTest)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberInvokeTest)
{
...
TEST_F(TestCodeDrivenModelViaMocks, EmberWriteAttributeAccessInterfaceReturningError)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, EmberWriteInvalidDataType)
{
    CodeDrivenDataModelProviderWithStorage model;
    ScopedMockAccessControl accessControl;
...
TEST_F(TestCodeDrivenModelViaMocks, DeviceTypeIteration)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, SemanticTagIteration)
{
    CodeDrivenDataModelProviderWithStorage model;
...
TEST_F(TestCodeDrivenModelViaMocks, ServerClusterInterfacesRegistration)
{
    TestServerClusterContext testContext;

    // Custom setup for CodeDrivenDataModelProvider to control the ServerClusterInterface
    // for the specific path under test.
    CodeDrivenDataModelProvider model;
    UseMockNodeConfig useMockConfig(gTestNodeConfig); // Ensure mock config is active
    InteractionModelContext imContext{
        .eventsGenerator         = nullptr, // Not used in this test
        .dataModelChangeListener = nullptr, // Not used in this test
        .actionContext           = nullptr, // Not used in this test
    };
    model.SetPersistentStorageDelegate(&testContext.StorageDelegate());
    ASSERT_EQ(model.Startup(imContext), CHIP_NO_ERROR);

    const ConcreteClusterPath kTestClusterPath(kMockEndpoint1, MockClusterId(2)); // The cluster we will customize

    FakeDefaultServerCluster fakeClusterServer(kTestClusterPath);

    static_assert(FakeDefaultServerCluster::kFakeClusterRevision != 0,
                  "Fake cluster revision should be non-zero for test validity");

    // Create an EndpointProvider for kMockEndpoint1
    // Add the FakeDefaultServerCluster for MockClusterId(2)
    const MockEndpointConfig * mockEp1Config = gTestNodeConfig.endpointById(kMockEndpoint1);
    ASSERT_NE(mockEp1Config, nullptr);

    std::vector<ServerClusterInterface *> serverClustersVec = { &fakeClusterServer };
    std::vector<DataModel::DeviceTypeEntry> deviceTypesVec;
    for (const auto & deviceType : mockEp1Config->deviceTypes())
    {
        deviceTypesVec.push_back(deviceType); // NOLINT(bugprone-use-emplace)
    }
    // For this specific test, client clusters and semantic tags are not the focus,
    // so we'll use empty spans.
    std::vector<ClusterId> clientClustersVec;
    std::vector<SemanticTag> semanticTagsVec;

    auto build_pair = SpanEndpointProvider::Builder(kMockEndpoint1)
                          .SetComposition(EndpointCompositionPattern::kFullFamily)
                          .SetServerClusters(Span(serverClustersVec.data(), serverClustersVec.size()))
                          .SetClientClusters(Span(clientClustersVec.data(), clientClustersVec.size()))
                          .SetSemanticTags(Span(semanticTagsVec.data(), semanticTagsVec.size()))
                          .SetDeviceTypes(Span(deviceTypesVec.data(), deviceTypesVec.size()))
                          .build();
    ASSERT_TRUE(std::holds_alternative<SpanEndpointProvider>(build_pair));
    auto ep1_provider_storage = std::make_unique<SpanEndpointProvider>(std::move(std::get<SpanEndpointProvider>(build_pair)));

    EndpointProviderRegistration ep1_registration(*ep1_provider_storage);
    ASSERT_EQ(model.AddEndpoint(ep1_registration), CHIP_NO_ERROR);
    uint32_t revision;
    ...
    // so we can see the right method is called.
    {
        ...
        ASSERT_TRUE(result->GetUnderlyingError() == CHIP_ERROR_INCORRECT_STATE);
    }
    ...
    // Cleanup: Remove the endpoint provider. This will also call Shutdown on its server clusters.
    ASSERT_EQ(model.RemoveEndpoint(kMockEndpoint1), CHIP_NO_ERROR);
    model.Shutdown();
}

TEST_F(TestCodeDrivenModelViaMocks, ServerClusterInterfacesListClusters)
{
    TestServerClusterContext testContext;
    UseMockNodeConfig config(gTestNodeConfig);

    CodeDrivenDataModelProvider model; // Use CodeDrivenDataModelProvider
    InteractionModelContext imContext = testContext.ImContext();
    model.SetPersistentStorageDelegate(&testContext.StorageDelegate());

    const EndpointId kTestEndpointId = kMockEndpoint1;
    const ClusterId kClusterId1      = MockClusterId(1); // A cluster that remains Codegen-backed
    const ClusterId kClusterId2      = MockClusterId(2); // The cluster we will customize with FakeDefaultServerCluster

    // Instances of ServerClusterInterface implementations
    ServerClusterShim codegen_c1({ { kTestEndpointId, kClusterId1 } });
    ServerClusterShim original_codegen_c2({ { kTestEndpointId, kClusterId2 } });

    // Prepare data for EndpointProvider from gTestNodeConfig
    const MockEndpointConfig * mockEpConfig = gTestNodeConfig.endpointById(kTestEndpointId);
    ASSERT_NE(mockEpConfig, nullptr);

    std::vector<ClusterId> client_clusters_vec;
    for (const auto & cluster_cfg : mockEpConfig->clusters)
    {
        if (cluster_cfg.emberCluster()->mask & MATTER_CLUSTER_FLAG_CLIENT)
        {
            client_clusters_vec.push_back(cluster_cfg.id);
        }
    }
    std::vector<DataModel::DeviceTypeEntry> device_types_vec(mockEpConfig->deviceTypes().begin(),
                                                             mockEpConfig->deviceTypes().end());
    std::vector<SemanticTag> semantic_tags_vec(mockEpConfig->semanticTags().begin(), mockEpConfig->semanticTags().end());

    // Initial EndpointProvider with Codegen-backed clusters
    std::vector<ServerClusterInterface *> initial_server_clusters_vec = { &codegen_c1, &original_codegen_c2 };
    auto initial_build_pair                                           = SpanEndpointProvider::Builder(kTestEndpointId)
                                  .SetComposition(EndpointCompositionPattern::kFullFamily)
                                  .SetServerClusters(Span(initial_server_clusters_vec.data(), initial_server_clusters_vec.size()))
                                  .SetClientClusters(Span(client_clusters_vec.data(), client_clusters_vec.size()))
                                  .SetSemanticTags(Span(semantic_tags_vec.data(), semantic_tags_vec.size()))
                                  .SetDeviceTypes(Span(device_types_vec.data(), device_types_vec.size()))
                                  .build();
    ASSERT_TRUE(std::holds_alternative<SpanEndpointProvider>(initial_build_pair));
    SpanEndpointProvider initial_ep_provider = std::move(std::get<SpanEndpointProvider>(initial_build_pair));

    EndpointProviderRegistration initial_ep_registration(initial_ep_provider);
    ASSERT_EQ(model.AddEndpoint(initial_ep_registration), CHIP_NO_ERROR);
    ASSERT_EQ(model.Startup(imContext), CHIP_NO_ERROR); // Start the model

    // Prepare FakeDefaultServerCluster and ensure its data version is different
    FakeDefaultServerCluster fakeClusterServer({ kTestEndpointId, kClusterId2 }); // Path for FakeDefaultServerCluster
    const ConcreteClusterPath kTestClusterPath(kTestEndpointId, kClusterId2);

    // version as we use this data version to differentiate between the two
    // The path for emberAfDataVersionStorage should match the one used by FakeDefaultServerCluster
    // which is {kTestEndpointId, kClusterId2} as defined in kTestClusterPath.
    ...
    // Get original cluster list (reflecting original_codegen_c2)
    ASSERT_EQ(model.ServerClusters(kTestEndpointId, builder), CHIP_NO_ERROR);
    ...
    // "Register" FakeDefaultServerCluster by replacing the endpoint provider
    ASSERT_EQ(model.RemoveEndpoint(kTestEndpointId), CHIP_NO_ERROR); // Shuts down clusters in initial_ep_provider

    std::vector<ServerClusterInterface *> updated_server_clusters_vec = { &codegen_c1, &fakeClusterServer };
    auto updated_build_pair                                           = SpanEndpointProvider::Builder(kTestEndpointId)
                                  .SetComposition(EndpointCompositionPattern::kFullFamily)
                                  .SetServerClusters(Span(updated_server_clusters_vec.data(), updated_server_clusters_vec.size()))
                                  .SetClientClusters(Span(client_clusters_vec.data(), client_clusters_vec.size()))
                                  .SetSemanticTags(Span(semantic_tags_vec.data(), semantic_tags_vec.size()))
                                  .SetDeviceTypes(Span(device_types_vec.data(), device_types_vec.size()))
                                  .build();
    ASSERT_TRUE(std::holds_alternative<SpanEndpointProvider>(updated_build_pair));
    SpanEndpointProvider updated_ep_provider = std::move(std::get<SpanEndpointProvider>(updated_build_pair));

    EndpointProviderRegistration updated_ep_registration(updated_ep_provider);
    ASSERT_EQ(model.AddEndpoint(updated_ep_registration), CHIP_NO_ERROR);

    // Get cluster list after "registration" (reflecting fakeClusterServer)
    ASSERT_EQ(model.ServerClusters(kTestEndpointId, builder), CHIP_NO_ERROR);
