...
{
public:
    CodeDrivenDataModelProviderWithStorage(const MockNodeConfig & nodeConfig = gTestNodeConfig) :
        ...
        })
    {
        ...
        for (const auto & endpointConfig : nodeConfig.endpoints)
        {
            auto ep_unique_ptr           = std::make_unique<DynamicEndpointProvider>( // Changed to DynamicEndpointProvider
                endpointConfig.id, static_cast<DataModel::EndpointCompositionPattern>(endpointConfig.composition));
            DynamicEndpointProvider * ep = ep_unique_ptr.get();

            CHIP_ERROR err = CHIP_NO_ERROR;

            for (const auto & deviceType : endpointConfig.deviceTypes())
            {
                err = ep->AddDeviceType(deviceType);
                VerifyOrDie(err == CHIP_NO_ERROR);
            }

            for (auto tag : endpointConfig.semanticTags())
            {
                err = ep->AddSemanticTag(tag);
                VerifyOrDie(err == CHIP_NO_ERROR);
            }

            for (const auto & clusterConfig : endpointConfig.clusters)
            {
                if (clusterConfig.emberCluster()->mask & MATTER_CLUSTER_FLAG_SERVER)
                {
                    auto serverCluster = std::unique_ptr<CodegenServerCluster>(
                        new CodegenServerCluster({ { endpointConfig.id, clusterConfig.id } }));
                    CodegenServerCluster * clusterPtr = serverCluster.get();
                    mServerClusterStorage.push_back(std::move(serverCluster));
                    err = ep->AddServerCluster(*clusterPtr);
                    VerifyOrDie(err == CHIP_NO_ERROR);
                }
                if (clusterConfig.emberCluster()->mask & MATTER_CLUSTER_FLAG_CLIENT)
                {
                    err = ep->AddClientCluster(clusterConfig.id);
                    VerifyOrDie(err == CHIP_NO_ERROR);
                }
            }
            err = AddEndpoint(*ep_unique_ptr);                            // Add the populated EndpointProviderInterface
            VerifyOrDie(err == CHIP_NO_ERROR);                            // Add the populated EndpointProviderInterface
            mEndpointProviderStorage.push_back(std::move(ep_unique_ptr)); // Store the EndpointProvider
        }
    ...
    DynamicEndpointProvider * GetEndpointProvider(EndpointId endpointId)
    {
        for (auto & ep : mEndpointProviderStorage)
        {
            if (ep->GetEndpointEntry().id == endpointId)
            {
                return ep.get();
            }
        }
...
public:                                   // Made public for test access
    std::vector<std::unique_ptr<ServerClusterInterface>> mServerClusterStorage;
    std::vector<std::unique_ptr<DynamicEndpointProvider>> mEndpointProviderStorage; // Declare the storage member
...
TEST_F(TestCodeDrivenModelViaMocks, InitializeCodeDrivenDataModelFromMockConfig)
{
    ...
    auto endpoints = endpointBuilder.TakeBuffer();
    ASSERT_EQ(endpoints.size(), gTestNodeConfig.endpoints.size());
    for (size_t i = 0; i < endpoints.size(); ++i)
    {
        EXPECT_EQ(endpoints[i].id, gTestNodeConfig.endpoints[i].id);
        EXPECT_EQ(endpoints[i].compositionPattern,
                  static_cast<EndpointCompositionPattern>(gTestNodeConfig.endpoints[i].composition));
        // Note: Parent ID is not stored in MockNodeConfig, assuming kInvalidEndpointId
        EXPECT_EQ(endpoints[i].parentId, kInvalidEndpointId);
    }
...
TEST_F(TestCodeDrivenModelViaMocks, ServerClusterInterfacesRegistration)
{
    ...
    // Create a DynamicEndpointProvider for kMockEndpoint1
    // Add the FakeDefaultServerCluster for MockClusterId(2)
    auto ep1_provider_storage = std::make_unique<DynamicEndpointProvider>(kMockEndpoint1, EndpointCompositionPattern::kFullFamily);
    DynamicEndpointProvider * ep1_provider = ep1_provider_storage.get();
    ASSERT_EQ(ep1_provider->AddServerCluster(fakeClusterServer), CHIP_NO_ERROR);
    ...
    {
        ASSERT_EQ(ep1_provider->AddDeviceType(deviceType), CHIP_NO_ERROR);
    }

    ASSERT_EQ(model.AddEndpoint(*ep1_provider), CHIP_NO_ERROR);
...
TEST_F(TestCodeDrivenModelViaMocks, ServerClusterInterfacesListClusters)
{
    ...
    // will add a fake cluster server which overrides the cluster data version
    // once registered
    ...
    // We know this is dynamic one, so we can cast it
    // Register the fake cluster server that overrides an ember one
    DynamicEndpointProvider * ep1_provider = model.GetEndpointProvider(kMockEndpoint1);
    ASSERT_NE(ep1_provider, nullptr);
    ASSERT_EQ(ep1_provider->AddServerCluster(fakeClusterServer), CHIP_NO_ERROR);
    ...
    EXPECT_EQ(originalClusters.size() + 1, afterRegistrationClusters.size());
    ...
    for (size_t i = 0; i < originalClusters.size(); i++)
    {
        ...
        const ServerClusterEntry & registered = afterRegistrationClusters[i + 1];
        ...
        {
            // TODO(sergiosoares): why is this failing?
            // Overall this test is assuming too much about the impolementation, likely not useful for the codedriven model.
