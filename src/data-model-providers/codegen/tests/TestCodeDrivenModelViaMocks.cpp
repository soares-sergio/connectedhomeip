...
public:                                   // Made public for test access
    ...
    std::vector<std::unique_ptr<EndpointProviderInterface>> mEndpointProviderStorage; // Declare the storage member
...
TEST_F(TestCodeDrivenModelViaMocks, ServerClusterInterfacesRegistration)
{
    ...
    // Get the EndpointProvider for kMockEndpoint1
    DynamicEndpointProvider * epProvider = nullptr;
    for (const auto & ep_unique_ptr : model.mEndpointProviderStorage)
    {
        if (ep_unique_ptr->GetEndpointEntry().id == kMockEndpoint1)
        {
            // In this test, we know that mEndpointProviderStorage contains DynamicEndpointProvider instances.
            epProvider = static_cast<DynamicEndpointProvider *>(ep_unique_ptr.get());
            break;
        }
    }
    ASSERT_NE(epProvider, nullptr);
    ASSERT_EQ(epProvider->RemoveServerCluster(MockClusterId(2)), CHIP_NO_ERROR);
    ASSERT_EQ(epProvider->AddServerCluster(fakeClusterServer), CHIP_NO_ERROR);
    ...
    ASSERT_EQ(epProvider->RemoveServerCluster(kTestClusterPath.mClusterId), CHIP_NO_ERROR);
    model.Shutdown();
}
